...
public class NaturalNumber3 extends NaturalNumberSecondary {
    ...
    public NaturalNumber3(String s) {
        ...
        int i = 0;
        /*
         * Count the number of leading zeros there are in the String, and start
         * the representation after the last leading zero.
         */
        while (s.charAt(i) == '0' && i < s.length() - 1) {
            i++;
        }
        this.rep = s.substring(i, s.length() - 1);
    }
    ...
    public final int divideBy10() {
        ...
        char returned = '0';
        /*
         * If the String is not empty, then returned is set to last character in
         * String (last digit).
         */
        if (!this.rep.isEmpty()) {
            returned = this.rep.charAt(this.rep.length() - 1);
            /*
             * If String length is greater than 1, the String becomes a
             * substring of itself, not including the last character (the final
             * digit).
             */
            if (this.rep.length() > 1) {
                this.rep = this.rep.substring(0, this.rep.length() - 1);
                /*
                 * Otherwise, the original String represent was one digit, so
                 * divide by 10 will result in zero. So, use createNewRep to
                 * make empty String to represent zero.
                 */
            } else {
                this.createNewRep();
            }
